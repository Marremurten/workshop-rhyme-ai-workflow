...
describe('CreateTaskForm', () => {
  ...
  it('clicking the button shows a title input', async () => {
    ...
    const input = screen.getByRole('textbox') || screen.getByPlaceholderText(/title|task/i);
  ...
  it('input clears after submit but form stays open (Trello-style)', async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn();
    ...
    expect(onSubmit).toHaveBeenCalledWith({ title: 'First task', column: 'todo' });

    // The input should still be visible (form stays open)
    const inputAfterSubmit = screen.getByRole('textbox');
    expect(inputAfterSubmit).toBeInTheDocument();

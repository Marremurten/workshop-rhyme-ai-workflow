...
function renderModal(taskOverrides: Partial<Task> = {}, props: Record<string, unknown> = {}) {
  const task = makeTask({ id: 1, title: 'Test task', description: 'Test description', ...taskOverrides });
  const defaultProps = {
    task,
    users: defaultUsers,
    onSave: vi.fn(),
...
describe('EditTaskModal', () => {
  ...
  it('renders with task description pre-filled in a textarea', () => {
    ...
    const descriptionTextarea = screen.getByDisplayValue('Users cannot log in after password reset');
  ...
  it('can change column from a dropdown', () => {
    ...
    const columnSelect = screen.getByRole('combobox', { name: /column|status/i })

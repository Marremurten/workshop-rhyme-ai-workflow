...
describe('LoginForm', () => {
  it('renders email and password inputs and a submit button', () => {
    ...
    expect(screen.getByRole('button', { name: /log\s*in|sign\s*in|submit/i })).toBeInTheDocument();
  });
  ...
  it('calls onLogin with email and password on submit', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /log\s*in|sign\s*in|submit/i }));
  ...
  it('has a link or button to switch to register mode', () => {
    ...
    const switchElement = screen.getByRole('button', { name: /register|sign\s*up|create account/i });
...
describe('RegisterForm', () => {
  ...
  it('calls onRegister with email, name, and password on submit', async () => {
    ...
    await user.type(screen.getByLabelText(/password/i), 'secret123');
    await user.click(screen.getByRole('button', { name: /register|sign\s*up|create account|submit/i }));
  ...
  it('shows error message on registration failure', async () => {
    const user = userEvent.setup();
    const onRegister = vi.fn().mockRejectedValue(new Error('Email already registered'));

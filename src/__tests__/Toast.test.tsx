...
// Helpers
// ---------------------------------------------------------------------------
function makeToast(overrides: Partial<ToastMessage> & { id: string; message: string }): ToastMessage {
  return {
    type: 'success',
    ...overrides,
  };
...
describe('Toast', () => {
  ...
  it('error toast does NOT auto-dismiss after 3 seconds', async () => {
    const onDismiss = vi.fn();
    const toasts: ToastMessage[] = [
      makeToast({ id: 'error-1', message: 'Something went wrong', type: 'error' }),
    ];
  ...
  it('clicking dismiss button calls onDismiss with the toast id', async () => {
    ...
    const dismissButton = screen.getByRole('button', { name: /close|dismiss|x|\u00d7/i });

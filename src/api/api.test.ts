...
describe("register", () => {
  ...
  it("throws an error with the server message on failure", async () => {
    ...
    await expect(register("b@c.com", "Bob", "secret")).rejects.toThrow(
...
describe("updateTask", () => {
  it("sends PUT to /api/tasks/:id with update data and credentials", async () => {
    ...
    const result = await updateTask(5, {
      title: "Updated",
      description: "New desc",
    });
  ...
  it("throws on 404 response", async () => {
    ...
    await expect(updateTask(999, { title: "Nope" })).rejects.toThrow(
      "Task not found",
    );
  });
});
...
describe("credentials and headers", () => {
  it("all GET requests include credentials:include", async () => {
    const getters = [() => fetchMe(), () => fetchUsers(), () => fetchTasks()];
